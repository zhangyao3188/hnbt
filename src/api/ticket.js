...
// Per-window key (same mechanism as grab.js)
let windowProxyKey = ''
...
ticket.interceptors.request.use((config) => {
  config.headers = config.headers || {}
  if (!config.headers['Content-Type']) {
    config.headers['Content-Type'] = 'application/json'
  }
  config.headers['AppPlatform'] = 'H5'
  // Send per-window key for per-window logging
...
ticket.interceptors.response.use(
  (resp) => resp.data,
  (error) => {
